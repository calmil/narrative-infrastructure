...
class Narrative(object):
    ...
    def __init__(self):
        self.symbolic_cycle_interval = 4
    ...
    def update(self, dt):
        ...
        if self.duration % cycle_interval == 0:
            narrator.counter(self.duration)

        if self.duration % (cycle_interval*self.symbolic_cycle_interval) == 0:
...
class Application():
    ...
    def retire(self, agent):
        print(str(agent.title) + ' is retired')
    ...
    def update(self, dt):
        ...
        if self.duration % (cycle_interval*retirement_cycles) == 0:
            self.retire(random.choice(agents))
