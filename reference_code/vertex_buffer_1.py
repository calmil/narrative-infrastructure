...
def render_rect_filled(shape, offset):
    ...
    # Initializes matrix to its origin before translating.
    GL.glLoadIdentity()
    # Set color
    GL.glColor3ub(shape.color[0], shape.color[1], shape.color[2])
    # Translate from initialized matrix, by multiplying current matrix by the arguments (x, y, z)
    GL.glTranslatef(shape.x + shape.width / 2, shape.y + shape.height / 2, 0)
    # Render primitives (mode, starting index, number of indices to render)
...
def create_rects(rect_list):
    ...
    # Here, we're going to need to do some work with making something C compatible in some capacity. I don't yet know why.
    # Generate buffer object names (number of names, destination array)
    GL.glGenBuffers(1, ctypes.pointer(vbo_id))

    data2 = (GL.GLfloat*len(v2f))(*v2f)

    # Binds a named buffer object (target, buffer)
    # In this case, the target is GL_ARRAY_BUFFER, which stores vertex attributes
    GL.glBindBuffer(GL.GL_ARRAY_BUFFER, vbo_id)
    # Creates a buffer objects 'data store' (target, size, data, usage)
